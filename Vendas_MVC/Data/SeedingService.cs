...
namespace Vendas_MVC.Data
{
    ...
    {
        /*Em primeiro lugar vou registar aqui uma dependencia desse seedingService com o nosso DbContext que é a classe Vendas_MVCContext 
         * Agora irei ter que declarar o meu Vendas_MVCContext  */

        private Vendas_MVCContext _context;


        /*Agora vou criar um constructor recebendo um objecto desse tipo (Vendas_MVCContext) para indicar que a injeccao dependencia deve acontecer   */
            public SeedingService(Vendas_MVCContext context)
        {
            _context = context; // Aqui 0 1 context é da classe recebendo como argumento
        }

        /*Criamos uma injecao de dependencia, ou seja, quando o "SeedingService" for criado
          automaticamente ele vai receber uma instancia do context tambem  */





        /* Onde eu vou chamar essa operação seed? Eu vou chamar dentro do startup e dentro do metodo configure, este metodo aceita que coloque outros parametros, e neste caso irei add lá "SeedingService" seedingService,
          e a classe SeedingService estiver registada no sistema de injecao de dependencia da aplicação que é justamente o caso aqui services.AddScoped<SeedingService>(); automaticamente vai ser resolvida uma instancia do objecto SeedingService */
        public void Seed() // Esta operação é que vai ser responsavel por popular a minha bd
        {
            // 1º teste - Se já existe algum dados na minha bd eu vou interromper a operação, para testar se ja existe algum dado eu vou testar da seg.forma 
            ...
            _context.Department.AddRange(d1, d2, d3, d4); /* Agora vou mandar add estes objectos no banco de dados, como fazemos isto usando EntityFramework? Vou chamar o objecto context.adicionando os departamentos */
            ...
            _context.SaveChanges(); // Salva e confirma as nossas alterações no banco de dados

            //Nota: Está pronto o nosso serviço de popular a nossa base de dados
